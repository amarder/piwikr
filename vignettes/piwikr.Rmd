...
Google Analytics is the industry standard for web analytics, but [Piwik][piwik] gives the analyst 100% data ownership. I like working with data so having access to the raw data from Piwik is a huge plus. Piwik stores all of its data in a MySQL database. To get started let's connect to that database using `RMySQL` and `dplyr`:
...
I've written an R library called `piwikr` to download and clean the tables stored in Piwik's database. Below I retrieve tables describing all visits to my site and all actions taken by visitors to the site.
...
My site has attracted `r nrow(visitors)` visitors over `r ndays` days. The average arrival rate is `r round(nrow(visitors) / ndays, 2)` visitors per day.

# Popular Content

What pages on my site have been viewed by the most visitors?
...
The data coming out of Piwik isn't perfect. I am using a redirect on one of my posts so [some browsers are losing referrer information][piwik-faq] and inflating the direct traffic numbers. If I wanted to fix this issue with Piwik I could remove that redirect.
