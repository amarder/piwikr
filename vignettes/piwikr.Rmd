...
```{r traffic, fig.height=3, fig.width=6}
...
```{r resolutions, fig.height=5, fig.width=6}
graph_browser_resolutions(visits)
pct_mobile <- (mean(visits$screen_width < 800, na.rm = TRUE) * 100) %>% round(1)
```

`r pct_mobile`% of visits were performed on a screen with width less than 800 pixels.
...
```{r structure, fig.width = 8}
set.seed(2)
graph_site_structure(actions, n = 14)
```
