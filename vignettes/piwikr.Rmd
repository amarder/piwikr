...
piwikr comes with functions to compute new tables from the primary tables. The four tables constructed below describe2 visitors to the site, days the site was actively collecting data, pages on the site, and sources of traffic to the site.
...
graph_browser_resolutions(visits) + theme(legend.position = c(0.9, 0.875))
...
set.seed(2)
graph_site_structure(actions, n = 14) + theme(legend.position = "bottom", legend.box = "horizontal")
```
